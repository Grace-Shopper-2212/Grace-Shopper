...
async function seed() {
	...
	//Creating Orders
	const formattedOrdersData = ORDER_SEED_DATA.map((order) => {
		order.userId = users[randomIntFromInterval(0, users.length - 1)].id;
		return order;
	});
	const orders = await Order.bulkCreate(formattedOrdersData, {
		validate: true,
	});

	// Filling some carts
	const formattedItemOrders = CART_SEED_DATA.map((position) => {
		position.orderId = orders[randomIntFromInterval(0, orders.length - 1)].id;
		position.itemId = items[randomIntFromInterval(0, items.length - 1)].id;
		return position;
	});
	const itemOrders = await Item_Order.bulkCreate(formattedItemOrders, {
		validate: true,
	});
